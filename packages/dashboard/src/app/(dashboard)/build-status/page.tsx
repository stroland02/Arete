...
import { FocusRail } from "@/components/dashboard/build-status/focus-rail";
import { Principles } from "@/components/dashboard/build-status/principles";
...
export default function BuildStatusPage() {
  ...
  return (
    <PageReveal className="space-y-8">
      ...
      <RevealItem>
        <FocusRail items={focusRail(tracker)} />
      </RevealItem>
      ...
      {(() => {
        const dropped = droppedItems(tracker);
        if (dropped.length === 0) return null;
        return (
          <RevealItem key="dropped">
            <Section
              title="Dropped"
...
function ItemRow({ item, titles }: { item: TrackerItem; titles: Map<string, string> }) {
  const { title, level, href, works, gap, evidence, area, blockedBy, provenance, state, droppedReason } = item;
  const isDropped = state === "dropped";

  return (
    <li className={cn("px-4 py-3.5", isDropped && "opacity-60 grayscale")}>
      <div className="flex flex-wrap items-center gap-2">
        {href ? (
          <Link
            href={href}
            className={cn(
              ...
              isDropped ? "line-through text-content-secondary hover:text-content-primary" : "text-content-primary hover:text-accent-primary"
            )}
          >
            {title}
          ...
          <span className={cn("text-sm font-medium", isDropped ? "line-through text-content-secondary" : "text-content-primary")}>
            {title}
          </span>
        )}
        <ReadinessBadge level={level} />
      ...
      {blockedBy && blockedBy.length > 0 ? (
        <p className="mt-1.5 text-[11px] text-content-muted">
          Blocked by{" "}
          {resolveBlockers(item).map((blocker, i) => {
            if (blocker.kind === "item") {
              return (
                <span key={blocker.id}>
                  {i > 0 ? ", " : ""}
                  <span className={blocker.done ? "line-through text-content-muted" : "font-medium text-content-secondary"}>
                    {blocker.title}
                  </span>
                </span>
              );
            ...
            if (blocker.kind === "external") {
              return (
                <span key={`ext-${i}`}>
                  {i > 0 ? ", " : ""}
                  <span className="italic">{blocker.text}</span>
            ...
            return (
              <span key={`unknown-${i}`}>
                {i > 0 ? ", " : ""}
                <span className="font-mono text-accent-warning">{blocker.id} (unknown)</span>
              </span>
            );
          })}
        </p>
      ...
      {isDropped && droppedReason ? (
        <p className="mt-2 rounded-md bg-content-primary/5 px-2 py-1.5 text-[11.5px] italic text-content-secondary border border-border-subtle">
          <span className="font-semibold not-italic">Dropped:</span> {droppedReason}
        </p>
      ) : null}
    </li>
  );
