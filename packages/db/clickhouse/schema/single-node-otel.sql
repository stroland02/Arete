CREATE DATABASE IF NOT EXISTS superlog;

-- otel_traces
CREATE TABLE IF NOT EXISTS superlog.otel_traces
(
...
;
-- otel_logs
CREATE TABLE IF NOT EXISTS superlog.otel_logs
(
    `Timestamp` DateTime64(9) CODEC(Delta(8), ZSTD(1)),
...
;
-- otel_metrics_gauge
CREATE TABLE IF NOT EXISTS superlog.otel_metrics_gauge
(
    `ResourceAttributes` Map(LowCardinality(String), String) CODEC(ZSTD(1)),
...
;
-- otel_metrics_sum
CREATE TABLE IF NOT EXISTS superlog.otel_metrics_sum
(
    `ResourceAttributes` Map(LowCardinality(String), String) CODEC(ZSTD(1)),
    `ResourceSchemaUrl` String CODEC(ZSTD(1)),
    `ScopeName` String CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS superlog.otel_metrics_summary
(
    `ResourceAttributes` Map(LowCardinality(String), String) CODEC(ZSTD(1)),
    `ResourceSchemaUrl` String CODEC(ZSTD(1)),
    `ScopeName` String CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS superlog.otel_metrics_histogram
(
    `ResourceAttributes` Map(LowCardinality(String), String) CODEC(ZSTD(1)),
    `ResourceSchemaUrl` String CODEC(ZSTD(1)),
    `ScopeName` String CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS superlog.otel_metrics_exp_histogram
(
    `ResourceAttributes` Map(LowCardinality(String), String) CODEC(ZSTD(1)),
    `ResourceSchemaUrl` String CODEC(ZSTD(1)),
    `ScopeName` String CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS superlog.otel_traces_trace_id_ts
(
    `TraceId` String CODEC(ZSTD(1)),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS superlog.otel_traces_trace_id_ts_mv TO superlog.otel_traces_trace_id_ts
(
    `TraceId` String,
    `Start` DateTime64(9),
    `End` DateTime64(9)
...
CREATE TABLE IF NOT EXISTS superlog.issue_activity_daily
(
...
CREATE MATERIALIZED VIEW IF NOT EXISTS superlog.issue_activity_daily_from_traces_mv TO superlog.issue_activity_daily
AS SELECT
...
CREATE MATERIALIZED VIEW IF NOT EXISTS superlog.issue_activity_daily_from_logs_mv TO superlog.issue_activity_daily
AS SELECT
    ResourceAttributes['superlog.project_id'] AS project_id,
...
CREATE TABLE IF NOT EXISTS superlog.events_per_minute
(
    `project_id` String CODEC(ZSTD(1)),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS superlog.events_per_minute_from_traces_mv TO superlog.events_per_minute
AS SELECT
    ResourceAttributes['superlog.project_id'] AS project_id,
    'traces' AS signal,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS superlog.events_per_minute_from_logs_mv TO superlog.events_per_minute
AS SELECT
    ResourceAttributes['superlog.project_id'] AS project_id,
    'logs' AS signal,
    ServiceName AS service,
...
CREATE TABLE IF NOT EXISTS superlog.otel_exceptions
(
    `project_id` String CODEC(ZSTD(1)),
    `Timestamp` DateTime64(9) CODEC(Delta(8), ZSTD(1)),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS superlog.otel_exceptions_from_traces_mv TO superlog.otel_exceptions
AS SELECT
    ResourceAttributes['superlog.project_id'] AS project_id,
    Timestamp,
    'span' AS kind,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS superlog.otel_exceptions_from_logs_mv TO superlog.otel_exceptions
AS SELECT
    ResourceAttributes['superlog.project_id'] AS project_id,
    Timestamp,
    'log' AS kind,
...
CREATE TABLE IF NOT EXISTS superlog.otel_traces_recent
(
    `project_id` String CODEC(ZSTD(1)),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS superlog.otel_traces_recent_mv
...
;
-- otel_traces_summary
CREATE TABLE IF NOT EXISTS superlog.otel_traces_summary
(
    `project_id` String CODEC(ZSTD(1)),
    `trace_id` String CODEC(ZSTD(1)),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS superlog.otel_traces_summary_mv
