...
// incident, so both the Incident detail page and the healing agent can see the
// real trace/log/exception context around an alert. This is dogfooding: our
// services emit the telemetry, our own surfaces read it back. (Distinct from
// telemetry/fetch-telemetry-context on the webhook side, which pulls a
// *customer's* external providers — PostHog, Sentry, Vercel — for PR review.)
//
// WHY IT LIVES IN `@arete/db` (moved here 2026-07-22 from
// `packages/dashboard/src/lib/telemetry-queries.ts`, which is now a thin
// re-export keeping every name and signature). It has TWO consumers now: the
// dashboard's Signals panel, and the healing path in `@arete/webhook`, which
// hands an incident's runtime context to the fix agent so a patch is authored
// against what actually failed rather than against static code evidence alone.
// These are PLATFORM-GATED reads, and the gate (`isPlatformInstallation`)
// already lives in this package. Per the tenancy contract §2's "one resolver,
// one truth" — and the identical argument that moved the gate itself here —
// two copies of a security-gated query are two places to drift, and drift here
// is a tenant leak. So the queries live beside the gate they depend on.
//
