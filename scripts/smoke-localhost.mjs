...
async function probe(port, path) {
  ...
  try {
    ...
    return { ok: res.ok, status: res.status, ms, authWalled, bytes: body.length };
  } catch (err) {
    return { down: true, reason: err.cause?.code ?? err.message };
  }
}

const PATHS = ["/", "/build-status", "/api/health"];
...
for (const port of PORTS) {
  ...
  if (git) {
    ...
    console.log("  serving   unknown checkout — cannot say which branch this is");
  }
